...
def match_brand(raw_text: Optional[str]) -> Optional[str]:
    if not raw_text:
        return None
    ...
    brands = list({s["brand"] for s in STORES})
    if not brands:
        return None

    norm_brands = [(b.lower(), b) for b in brands if b]

    norm_candidates = [nb for nb, _ in norm_brands]

    best = process.extractOne(text, norm_candidates, scorer=fuzz.partial_ratio)
    if not best:
        return None
...
def match_city(raw_text: Optional[str]) -> Optional[str]:
    if not raw_text:
        return None
    ...
    cities = list({s["city"] for s in STORES})
    if not cities:
        return None

    words = re.findall(r"[a-zа-яёіїєґ']+", text, flags=re.IGNORECASE)
    ...
    for w in words:
        for city in cities:
            ...
            prefix_len = _common_prefix_len(w, c_lower)
            if prefix_len == 0:
                continue

            fuzzy_score = fuzz.ratio(w, c_lower)

            if (
                prefix_len > best_prefix
                or (prefix_len == best_prefix and fuzzy_score > best_fuzzy)
            ):
